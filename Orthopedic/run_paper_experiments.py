...
def _require_peak_sigma_config() -> tuple[float, float]:
    ...
    if peak is None or sigma_read is None:
        raise ValueError(
            ...
            "`estimate_peak` and `estimate_sigma_read` into Orthopedic/data.py."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run end-to-end JHU paper experiments."
    )
    parser.add_argument("--root", default=str(SCRIPT_DIR))
    parser.add_argument("--dataset", default="jhu", choices=["jhu"])

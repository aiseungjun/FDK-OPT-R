...
JHU_VAL_PATIENTS = {"cadaver_18-2799"}
JHU_TEST_PATIENTS = {"cadaver_17-1905"}
_JHU_FRAME_PAT = re.compile(r"^(\d+)")
JHU_DOWNSAMPLE_FROM_HW = (384, 384)
JHU_DOWNSAMPLE_TO_HW = (192, 192)
...
def _apply_jhu_resolution_rule(x: torch.Tensor) -> torch.Tensor:
    if x.dim() != 3:
        raise ValueError(f"Expected CHW tensor, got shape {tuple(x.shape)}")
    if tuple(x.shape[-2:]) == JHU_DOWNSAMPLE_FROM_HW:
        return _resize_chw(x, JHU_DOWNSAMPLE_TO_HW)
    return x
...
def _infer_dataset_kind(root: Path) -> str:
    if (
        (root / "jhu").exists()
        or (root / "JHU").exists()
        or any(root.glob("cadaver_*"))
    ):
        return "jhu"
    raise FileNotFoundError(
        f"Could not infer JHU dataset root from: {root}. "
        "Expected 'jhu/' or 'cadaver_*' patient folders."
    )
...
def _list_jhu_patients(root: Path) -> list[Path]:
    base = _jhu_root(root)
    return sorted([p for p in base.glob("cadaver_*") if p.is_dir()])


def _split_jhu_patients(root: Path, split: str) -> list[Path]:
    patients = _list_jhu_patients(root)
    if split == "validation":
        return [p for p in patients if p.name in JHU_VAL_PATIENTS]
    if split == "test":
        return [p for p in patients if p.name in JHU_TEST_PATIENTS]
    if split == "training":
        excluded = JHU_VAL_PATIENTS | JHU_TEST_PATIENTS
        return [p for p in patients if p.name not in excluded]
    raise ValueError(f"Unsupported split: {split}")


def _list_preview_dirs(patient_dir: Path) -> list[Path]:
    fluoro = patient_dir / "fluoro"
    if not fluoro.exists():
        return []
    return sorted([p for p in fluoro.rglob("preview") if p.is_dir()])


def _list_preview_images(preview_dir: Path) -> list[Path]:
    files: list[Path] = []
...
def _jhu_frame_idx(path: Path, fallback: int) -> int:
    m = _JHU_FRAME_PAT.match(path.stem)
    if m:
        try:
...
def _group_jhu_frames(
    ...
    for patient_dir in _split_jhu_patients(root, split):
        for preview_dir in _list_preview_dirs(patient_dir):
            files = _list_preview_images(preview_dir)
            if not files:
                continue
            files = sorted(files, key=lambda p: (_jhu_frame_idx(p, 10**9), str(p)))
            ...
            for local_idx, path in enumerate(files):
                frame_idx = _jhu_frame_idx(path, local_idx)
                ref = FrameRef(dataset=seq_key, index=local_idx)
                items.append((frame_idx, ref))
                path_map[(seq_key, local_idx)] = path
...
def _load_frame_path(root_path: Path, ref: FrameRef) -> Path:
    ...
    groups, path_map = _group_jhu_frames(root_path, split="training")
    if ref.dataset not in groups:
        all_map = {}
        for split in ("training", "validation", "test"):
            _, m = _group_jhu_frames(root_path, split=split)
            all_map.update(m)
    ...
    *,
    ...
    if ref.dataset not in groups:
        ...
        for split_name in ("training", "validation", "test"):
            g, m = _group_jhu_frames(root_path, split=split_name)
            all_groups.update(g)
            all_map.update(m)
        groups = all_groups
        path_map = all_map
    ...
    if tensor.max() > 1.5:
        tensor = tensor / 255.0
    tensor = _apply_jhu_resolution_rule(tensor)
    if bbox is None:
...
def get_frame_groups(
    ...
    *,
    ...
    if kind == "auto":
        kind = "jhu"
    if kind != "jhu":
        raise ValueError(f"Unsupported dataset kind for Orthopedic: {dataset_kind}")
    _infer_dataset_kind(root_path)
    groups, _path_map = _group_jhu_frames(root_path, split)
    return groups
    ...
    def __init__(
        ...
        *,
        ...
        if self.dataset_kind == "auto":
            self.dataset_kind = "jhu"
        if self.dataset_kind != "jhu":
            raise ValueError(
                f"Unsupported dataset_kind for Orthopedic: {self.dataset_kind}"
            )
        ...
        groups, path_map = _group_jhu_frames(self.root, self.split)
    ...
    def _validate_opt_index_coverage(self) -> None:
        ...
        if missing:
            ...
            raise RuntimeError(
                ...
                "Regenerate opt labels with current JHU training/validation split."
            )

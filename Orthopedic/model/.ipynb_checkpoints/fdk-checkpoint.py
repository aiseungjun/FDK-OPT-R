...
class FDK(nn.Module):
    ...
    def __init__(
        ...
        **_kwargs,
        ...
        in_c = 3
        layers: list[nn.Module] = [nn.Conv2d(in_c, self.width, 3, padding=1), nn.ReLU(inplace=True)]
        for _ in range(max(0, self.blocks - 1)):
            layers += [nn.Conv2d(self.width, self.width, 3, padding=1), nn.ReLU(inplace=True)]
    ...
    def _kalman_update(self, mu: torch.Tensor, target: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
    ...
    def _predict_residual(self, y0: torch.Tensor, v: torch.Tensor, target: torch.Tensor, mu: torch.Tensor) -> torch.Tensor:

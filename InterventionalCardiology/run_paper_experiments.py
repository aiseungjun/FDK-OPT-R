...
def _infer_dataset_kind(root: Path, dataset: str) -> str:
    dataset = dataset.lower()
    if dataset != "cardiac":
        raise ValueError(f"Unsupported dataset for InterventionalCardiology: {dataset}")
    if (root / "cardio_data" / "anonymous_syntax_holdout").exists() or (
        root / "anonymous_syntax_holdout"
    ).exists():
        return "cardiac"
    if (root / "cardio_data").exists():
        return "cardiac"
    raise ValueError(
        f"Could not infer cardiac dataset kind from root={root} and dataset={dataset}"
    )
...
def _require_peak_sigma_config() -> tuple[float, float]:
    ...
    if peak is None or sigma_read is None:
        raise ValueError(
            ...
            "`estimate_peak` and `estimate_sigma_read` into InterventionalCardiology/data.py."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run end-to-end cardiac paper experiments."
    )
    parser.add_argument("--root", default=str(SCRIPT_DIR))
    parser.add_argument("--dataset", default="cardiac", choices=["cardiac"])
    ...
    global TRAIN_BATCH_FRACTION
    TRAIN_BATCH_FRACTION = float(args.train_batch_fraction)

    root = Path(args.root)
    datasets = [_infer_dataset_kind(root, args.dataset)]
    ...
    for dataset in datasets:
        ...
        opt_epochs = int(args.opt_epochs)
        patch_size = int(args.patch_size)
        if dataset == "cardiac":
            if base_epochs != 50:
                print(
                    f"[info] Cardiac baseline epochs fixed at 50; ignoring epochs={base_epochs}."
                )
            if opt_epochs != 30:
                print(
                    f"[info] Cardiac fdk stage epochs fixed at 30; ignoring opt_epochs={opt_epochs}."
                )
            if patch_size != 256:
                print(
                    f"[info] Cardiac patch size fixed at 256; ignoring patch_size={patch_size}."
                )
            if args.lr_step_patients != 0:
                print(
                    f"[info] Cardiac LR step uses epoch-based schedule; ignoring lr_step_patients={args.lr_step_patients}."
                )
            base_epochs = 50
            opt_epochs = 30
            patch_size = 256
            args.lr_step_patients = 0
        ...
        for run_idx, run_seed in enumerate(run_seeds, start=1):
            ...
            _train_one(
                ...
                lr_step_patients=args.lr_step_patients,

...
def _infer_dataset_tag(root: Path, dataset_kind: str = "cardiac") -> str:
    if dataset_kind.lower() != "cardiac":
        raise ValueError(f"Unsupported dataset kind: {dataset_kind}")
    if (root / "cardio_data" / "anonymous_syntax_holdout").exists() or (
        root / "anonymous_syntax_holdout"
    ).exists():
        return "cardiac"
    raise ValueError(
        f"Could not infer cardiac dataset kind from root={root}. "
        "Expected cardiac dataset directories."
    )


def _count_cardiac_train_patients(root: Path) -> int:
    try:
        groups = get_frame_groups(
            root=root, split="training", dataset_kind="cardiac", pretrain=False
        )
    except Exception:
        return 0
    if not groups:
    ...
    for key in groups:
        key_text = str(key)
        patient_ids.add(key_text.rsplit("__", 1)[0] if "__" in key_text else key_text)
    return len(patient_ids)


def _resolve_lr_step_size(
    train_loader,
    ...
    if args.lr_step_size <= 0:
        return None
    if args.lr_step_patients <= 0 or dataset_tag != "cardiac":
        return int(args.lr_step_size)

    train_patients = _count_cardiac_train_patients(Path(root))
    if train_patients <= 0:
        return int(args.lr_step_size)
    ...
    batch_size = max(
        1, int(train_loader.batch_size) if train_loader.batch_size is not None else 1
    )

    frames_per_patient = float(dataset_len) / float(train_patients)
    iter_per_patient = frames_per_patient / float(batch_size)
    step = math.ceil(iter_per_patient * float(args.lr_step_patients))
    if step <= 0:
        step = 1
    return step
...
def _resolve_max_train_batches(loader, fraction: float) -> int | None:
    frac = float(fraction)
    if frac <= 0.0:
        raise ValueError(f"train_batch_fraction must be > 0, got {frac}")
    if frac >= 1.0:
        return None
    total_batches = len(loader)
    if total_batches <= 0:
        return None
    return max(1, int(math.ceil(total_batches * frac)))


def _resolve_sample_budget(sample_budget: int | None, dataset_size: int) -> int | None:
    ...
    *,
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if max_samples is not None:
            remaining = int(max_samples) - int(samples_seen)
            if remaining <= 0:
                break
            if inp.size(0) > remaining:
                inp = inp[:remaining]
...
def _train_one_epoch_wgan(
    ...
    *,
    ...
    g_scheduler: StepLR | None = None,
    d_scheduler: StepLR | None = None,
    max_batches: int | None = None,
    max_samples: int | None = None,
) -> tuple[float, float]:
    generator.train()
...
def main():
    ...
    parser.add_argument("--lr_step_patients", type=int, default=0)
    parser.add_argument("--lr_gamma", type=float, default=0.2)
    parser.add_argument("--skip_val", action="store_true")
    parser.add_argument("--train_batch_fraction", type=float, default=1.0)
    parser.add_argument("--max_train_samples_per_epoch", type=int, default=None)
    parser.add_argument("--max_val_samples_per_epoch", type=int, default=None)
    ...
    if args.train_batch_fraction <= 0.0 or args.train_batch_fraction > 1.0:
        raise ValueError("train_batch_fraction must be in (0, 1].")
    ...
    if dataset_tag == "cardiac":
        if int(args.patch_size) != int(CARDIAC_PATCH_SIZE):
            print(
                f"[info] Cardiac experiments use patch_size={CARDIAC_PATCH_SIZE}; "
                f"ignoring patch_size={args.patch_size}."
            )
        args.patch_size = int(CARDIAC_PATCH_SIZE)
        if args.max_train_samples_per_epoch is None:
    ...
    if isinstance(model, (FDK, UNetTemp)):
        ...
        if stage1_max_samples is not None:
            print(
                f"[info] Stage1 train sample budget per epoch: "
                f"{stage1_max_samples}/{len(train_loader.dataset)}"
            )
        if stage1_val_max_samples is not None:
            print(
        ...
        step_size = _resolve_lr_step_size(
            train_loader, args, dataset_tag, Path(args.root)
        )
        scheduler = (
            StepLR(opt, step_size=step_size, gamma=args.lr_gamma)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if not args.skip_val:
                ...
                print(
                    f"[{stage_prefix}-stage1] Epoch {epoch}/{args.epochs} - train: {train_loss:.6f}"
                )
        ...
        if stage2_max_samples is not None:
            print(
                f"[info] Stage2 train sample budget per epoch: "
                f"{stage2_max_samples}/{len(train_loader.dataset)}"
            )
        if stage2_val_max_samples is not None:
            print(
        ...
        def _run_temporal_epochs(
            ...
            scheduler2 = (
                StepLR(opt2, step_size=step_size, gamma=args.lr_gamma)
                if step_size is not None
                else None
            )
            for epoch in range(1, epochs + 1):
                ...
                for batch_idx, batch in enumerate(train_loader):
                    ...
                    if stage2_max_samples is not None:
                        remaining = int(stage2_max_samples) - int(samples_seen)
                        if remaining <= 0:
                            break
                        if inp.size(0) > remaining:
                            inp = inp[:remaining]
                    ...
                    count += 1
                    samples_seen += current_bs
                    if stage2_max_samples is not None and samples_seen >= int(
                        stage2_max_samples
                    ):
                        break
                    if stage2_max_batches is not None and (batch_idx + 1) >= int(
                        stage2_max_batches
                    ):
                        break
        ...
        if wgan_max_samples is not None:
            print(
                f"[info] Train sample budget per epoch: "
                f"{wgan_max_samples}/{len(train_loader.dataset)}"
            )
        if wgan_val_max_samples is not None:
            print(
        ...
        g_scheduler = (
            StepLR(g_opt, step_size=step_size, gamma=args.lr_gamma)
            if step_size is not None
            else None
        )
        d_scheduler = (
            StepLR(d_opt, step_size=step_size, gamma=args.lr_gamma)
            if step_size is not None
            else None
        )
        ...
        for epoch in range(1, args.epochs + 1):
            g_loss, d_loss = _train_one_epoch_wgan(
                ...
                max_batches=wgan_max_batches,
                max_samples=wgan_max_samples,
            )
            if not args.skip_val:
                val_loss = _eval_one_epoch(
                    model,
                    ...
                    opt_l1_weight=0.0,
                    opt_ssim_weight=0.0,
                    max_samples=wgan_val_max_samples,
                )
                print(
                ...
                print(f"Epoch {epoch}/{args.epochs} - G: {g_loss:.6f} D: {d_loss:.6f}")
        ...
        if max_train_samples is not None:
            print(
                f"[info] Train sample budget per epoch: "
                f"{max_train_samples}/{len(train_loader.dataset)}"
            )
